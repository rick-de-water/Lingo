...
function(unicode_generate_mapping_header FROM TO FROM_NAME TO_NAME OUTPUT_VARIABLE)
	...
	string(APPEND HEADER "\t\t\ttemplate <typename Point>\n")
	string(APPEND HEADER "\t\t\tstruct ${FROM_NAME}_${TO_NAME}_mapping\n\t\t\t{\n")
	string(APPEND HEADER "\t\t\t\tusing point_type = Point;\n\n")

	unicode_list_to_c_array(MINOR_TABLE_ARRAY ${EMPTY_MINOR_TABLE})
	string(APPEND HEADER "\t\t\t\tstatic constexpr point_type minor_table_empty[${MINOR_TABLE_SIZE}] = ${MINOR_TABLE_ARRAY};\n")
	...
	foreach(MINOR_TABLE_INDEX RANGE ${MINOR_TABLE_RANGE})
		if (DEFINED "MINOR_TABLE_${MINOR_TABLE_INDEX}")
			...
			string(APPEND HEADER "\t\t\t\tstatic constexpr point_type minor_table_${MINOR_TABLE_INDEX}[${MINOR_TABLE_SIZE}] = ${MINOR_TABLE_ARRAY};\n")
		endif()
	endforeach()
	string(APPEND HEADER "\n")

	string(APPEND HEADER "\t\t\t\tstatic constexpr point_type point_start_index = ${POINT_INDEX_START};\n")
	string(APPEND HEADER "\t\t\t\tstatic constexpr point_type point_end_index = ${POINT_INDEX_END};\n\n")

	string(APPEND HEADER "\t\t\t\tstatic constexpr point_type minor_table_start_index = ${MINOR_TABLE_START};\n")
	string(APPEND HEADER "\t\t\t\tstatic constexpr point_type minor_table_end_index = ${MINOR_TABLE_END};\n")
	string(APPEND HEADER "\t\t\t\tstatic constexpr point_type minor_table_count = ${MINOR_TABLE_COUNT};\n\n")

	string(APPEND HEADER "\t\t\t\tstatic constexpr const point_type* major_table[${MINOR_TABLE_COUNT}] = { ")
	foreach(MINOR_TABLE_INDEX RANGE ${MINOR_TABLE_RANGE})
		if (DEFINED "MINOR_TABLE_${MINOR_TABLE_INDEX}")
	...
	string(APPEND HEADER "\t\t\t};\n")
	string(APPEND HEADER "\n")
	string(APPEND HEADER "\t\t\ttemplate <typename Point> constexpr Point ${FROM_NAME}_${TO_NAME}_mapping<Point>::minor_table_empty[${MINOR_TABLE_SIZE}];\n")
	foreach(MINOR_TABLE_INDEX RANGE ${MINOR_TABLE_RANGE})
		if (DEFINED "MINOR_TABLE_${MINOR_TABLE_INDEX}")
			unicode_list_to_c_array(MINOR_TABLE_ARRAY ${MINOR_TABLE_${MINOR_TABLE_INDEX}})
			string(APPEND HEADER "\t\t\ttemplate <typename Point> constexpr Point ${FROM_NAME}_${TO_NAME}_mapping<Point>::minor_table_${MINOR_TABLE_INDEX}[${MINOR_TABLE_SIZE}];\n")
		endif()
	endforeach()
	string(APPEND HEADER "\t\t\ttemplate <typename Point> constexpr const Point* ${FROM_NAME}_${TO_NAME}_mapping<Point>::major_table[${MINOR_TABLE_COUNT}];\n")
	string(APPEND HEADER "\t\t}\n")
	string(APPEND HEADER "\t}\n")
	string(APPEND HEADER "}\n\n")
